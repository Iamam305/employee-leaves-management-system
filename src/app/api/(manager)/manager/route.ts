...
export const POST = async (req: NextRequest) => {
    try {
      ...
      if (auth[0] === null || ["employee", "manager"].includes(auth[0]?.membership?.role)) {
      ...
      if (!managerMembership) {
        return NextResponse.json({ msg: "Manager not found in the organization" }, { status: 404 });
      }
  
      // Update all users with the new manager_id
      ...
      if (result.nModified === 0) {
        return NextResponse.json({ msg: "No users were updated" }, { status: 404 });
      }
  
      // Respond with success message
      return NextResponse.json({ msg: "Manager assigned to users successfully" }, { status: 200 });
    } catch (error) {
      console.error(error);
...
export const GET = async (req: NextRequest) => {
    try {
      // Check authentication
      const auth: any = await auth_middleware(req);
      const org_id = req.nextUrl.searchParams.get("org_id") || auth[0]?.membership?.org_id;
      ...
      } else if (["hr", "manager" , "employee"].includes(auth_data.membership.role)) {
