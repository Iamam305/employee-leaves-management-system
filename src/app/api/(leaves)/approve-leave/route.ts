...
import { restoreLeaveBalance, updateLeaveBalance } from "@/lib/balanceservices";
  ...
  try {
    ...
    if (auth[0] === null || auth[0]?.membership?.role === "employee") {
      return NextResponse.json(
        { msg: "unauthorized you dont have rights to approve leave" },
        { status: 401 }
      );
    }
    ...
    if (statusupdate === "rejected") {
      ...
      updateuserbalance = await restoreLeaveBalance(
        updateleave.user_id,
        year,
        updateleave.leave_type_id.name,
        month,
        days
      );
    }
...
export const GET = async (req: NextRequest) => {
  try {
    ...
    return NextResponse.json(
      { msg: "All Leaves fetched Successfully", data: leaves },
      { status: 200 }
    );
  } catch (error) {
    console.log(error);
