...
export const initializeEmployeeBalance = async (
  userId: string,
  year: any,
  orgId: string
) => {
...
export const calculateLeaveBalance = async (
  ...
  const monthlyBalances = Object.fromEntries(
    Array.from(leaveTypeBalance.monthly.entries()).map(
      ([month, balance]: any) => [
        month,
        {
...
export const updateLeaveTypeForAllEmployees = async (
  ...
  if (!allBalances || allBalances.length === 0) {
    throw new Error(
      "No balances found for the specified organization and year"
    );
  }
  ...
  for (const balance of allBalances) {
    ...
    if (!balance.leaveBalances.has(oldLeaveTypeName)) {
      console.log(
        `Leave type ${oldLeaveTypeName} not found for user ${balance.userId}`
      );
      continue; // Skip to the next employee if leave type is not found
    }

    // Get the current balance for the old leave type
    const oldLeaveTypeBalance: any =
      balance.leaveBalances.get(oldLeaveTypeName);
    ...
    const newLeaveTypeBalance = {
      ...
      monthly: new Map<
        string,
        { credit: number; used: number; available: number }
      >(),
    };
    ...
    for (let month = 0; month < 12; month++) {
      ...
      if (!currentMonthBalance) {
        throw new Error(
          `Month ${month} not found for leave type ${oldLeaveTypeName}`
        );
      }
...
export const restoreLeaveBalance = async (
  ...
  const currentMonthBalance = leaveTypeBalance.monthly.get(monthKey);
  const newMonthUsed = currentMonthBalance.used - days;
  const newMonthAvailable = currentMonthBalance.available + days;
  leaveTypeBalance.monthly.set(monthKey, {
    credit: currentMonthBalance.credit,
    used: newMonthUsed,
    available: newMonthAvailable,
  });
  const totalUsed = leaveTypeBalance.total.used - days;
  const totalAvailable = leaveTypeBalance.total.available + days;
  leaveTypeBalance.total.used = totalUsed;
  leaveTypeBalance.total.available = totalAvailable;
  balance.leaveBalances.set(leaveTypeName, leaveTypeBalance);
  await balance.save();
